listCov <- function(x, eres, unexpect, scaleparam, rhoparam, ltps, sampsize, corstr, scaled){

  if (corstr=="ind"){
    if (unexpect == 0.5) {
      ephi = rep(1, length(eres))
    } else {
      ephi = asymweight(eres, unexpect)
    }
    Memat =  0
    Brmat = 0
    MematMod =  0
    ...
    for(i in 1:sampsize ){
      ...
      xi = x[itps, ]
      sigmahat = tcrossprod(ires*iphi)
      ivcovmat = ivcov(scaleparam, rhoparam, length(itps), corstr, scaled)
